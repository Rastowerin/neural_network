...
for row in tables[0].find_all('tr')[2:]:
  elements[str(row.find_all('td'))[5: -6]] = {'удельная теплоемкость': float(str(row.find_all('th')[0])[4: -5].replace(',', '.').replace('–', '-'))*1000,
                                              'темпиратура плавления': float(str(row.find_all('th')[1])[4: -5].replace(',', '.').replace('–', '-')),
                                              'удельная теплота плавления': float(str(row.find_all('th')[2])[4: -5].replace(',', '.').replace('–', '-'))*1000}

for row in tables[1].find_all('tr')[2:]:
  elements[str(row.find_all('td'))[5: -6]] = {'удельная теплоемкость': float(str(row.find_all('th')[0])[4: -5].replace(',', '.').replace('–', '-'))*1000,
                                              'темпиратура кипения': float(str(row.find_all('th')[1])[4: -5].replace(',', '.').replace('–', '-')),
                                              'удельная теплота парообразования': float(str(row.find_all('th')[2])[4: -5].replace(',', '.').replace('–', '-'))*1000000}
